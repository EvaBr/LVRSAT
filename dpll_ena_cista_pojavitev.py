#implementacija DPLL brez (oz. z le eno) ciste pojavitve
                ...
                while crka < koliko:
                        ...
                        if naVrsti.ime == Abjikt:
                                ...
                                if (Tip==Til and vridnastAbjikta==T()) or (Tip==Lit and vridnastAbjikta==F()): #vstavljamo F v Ali -> lahko ga kar spustimo.
                                        ...
                                        koliko -= 1
                                        crka -= 1
                                else: #if (Tip==Til and vridnastAbjikta==F()) or (Tip==Lit and vridnatAbjikta==T()) #vstavljamo T v Ali -> lahko spustimo ce$
...
def dpll_brez_ciste(dieFormel):
        """ Sprejme formulo v CNF obliki in pove, ali ji je mogoce zadostiti. 
                Ce ji je, vrne slovar potrebnih vrednosti spremenljivk. """

        slovarc = {}

        ### Prva (in edina) cista pojavitev; takoalitako si je treba zapomniti vse nastopajoce spremenljivke, zato zaradi nje ne bo trajalo pretirano dlje:
        pojavitve = {}
        ...
        def pomozna(formula, slovar):
                ...
                if nasliNovo: #to se mora zgodit zmeraj
                        formula2 = Cnf([Stavek([litral for litral in st.literali]) for st in formula.stavki]) #naredimo kopijo, saj zadevo kasneje spreminja$
