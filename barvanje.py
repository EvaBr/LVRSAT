...
def kbarvanje(K, n, E):
    ...
    #vsako vozlišče je pobarvano vsaj z eno barvo
    for i in range(1,n+1):
        prva = In(prva, Ali(*tuple(Spr(("c{0}.{1}").format(i,j)) for j in range (1,K+1))))
    prva = prva.poenostavi()

    #eno vozlišče ni pobarvano z več kot eno barvo
    for i in range(1,n+1):
        druga = In(druga, In(*tuple (Ali(Neg(Spr(("c{0}.{1}").format(i,j))),Neg(Spr(("c{0}.{1}").format(i,k)))) for j in range (1,K+1) for k in range (j+1,K+1))))
    druga = druga.poenostavi()
    
    #sosednji vozlišči sta različnih barv
    for pov in E:
        tretja = In(tretja, In(*tuple(Ali(Neg(Spr(("c{0}.{1}").format(pov[1],j))),Neg(Spr(("c{0}.{1}").format(pov[0],j)))) for j in range (1,K+1))))
    tretja = tretja.poenostavi()
